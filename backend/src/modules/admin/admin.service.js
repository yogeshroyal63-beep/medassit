...
async function auditLogs({ page = 1, limit = 50 } = {}) {
  const skip = (page - 1) * limit;
  const [triages, triageTotal, appointments, appointmentTotal] = await Promise.all([
    Triage.find({}).sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
    Triage.countDocuments({}),
    Appointment.find({}).sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
    Appointment.countDocuments({}),
  ]);
  return {
    triages,
    appointments,
    total: { triages: triageTotal, appointments: appointmentTotal },
    page,
    limit,
  };
}
