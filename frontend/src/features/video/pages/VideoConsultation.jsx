...
// NOTE: This component only supports 1:1 peer connections (peers[0]).
// Group calls are not implemented — this is a known limitation, not a bug.
...
const VideoConsultation = () => {
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      ...
      let room;
      try { const { data } = await api.post("/video/create-room", { appointmentId: searchParams.get("appointmentId") || undefined }); room = data; setRoomInfo(data); }
      catch { toast.error("Could not create video room"); setLoading(false); return; }
      // Pass the JWT in the Socket.io handshake so the server can authenticate
      // the connection before allowing the user to join any room.
      const token = localStorage.getItem("medassist_token");
      const socket = io(BACKEND_URL, { transports: ["websocket"], auth: { token } });
      socketRef.current = socket;
      socket.on("connect", () => socket.emit("join-room", room.roomId));
      socket.on("room-full", ({ message }) => { toast.error(message || "Room is full"); setLoading(false); });
