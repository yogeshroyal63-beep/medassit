...
const ACTION_STYLE = {
  confirmed: { background: "#ccfbf1", color: "#065f46" },
...
const DoctorPortal = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      const fee      = profileRes.status === "fulfilled"
        ? (profileRes.value.data?.consultationFee ?? 500)
        : 500;
  ...
  if (loading) return (
    <div>
      <div className="stat-grid">
        {[1,2,3,4].map(i => (
          <div key={i} style={{ height: "88px", borderRadius: "12px", background: "var(--border)", animation: "pulse-ring 1.5s infinite" }} />
        ))}
      </div>
    ...
    <div>
      ...
      <div className="section-card">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
            ...
            return (
              <div key={a._id} style={{
                ...
                <div>
                  ...
                  {/* formatDate used here — no more raw ISO timestamp */}
                  <p style={{ margin: 0, fontSize: "12px", color: "var(--text-muted)" }}>
                    {formatDate(a.date)} · {a.time}
                  </p>
                  <span className={`badge ${badgeClass}`} style={{ textTransform: "capitalize", marginTop: "4px" }}>
                    {a.status}
                  </span>
                </div>
                {actions.length > 0 && (
          ...
          {!filtered.length && (
            <p style={{ color: "var(--text-subtle)", fontSize: "13px" }}>No appointments found.</p>
          )}
        </div>
