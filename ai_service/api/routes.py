...
def submit_feedback(request: FeedbackRequest, http_request: Request):
    ...
    try:
        ...
        # Log the error but don't crash the request — feedback is non-critical
        import logging
        logging.getLogger(__name__).error("Failed to write feedback log: %s", exc)
        raise HTTPException(status_code=500, detail="Could not save feedback. Please try again later.")
