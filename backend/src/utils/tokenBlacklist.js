 ...
 *
 * SECURITY NOTE: has() fails-CLOSED on Redis errors. A revoked token is
 * treated as blacklisted (denied) during a Redis outage. This is the
 * correct behaviour for a healthcare application.
 */
  ...
  return {
    async add(jti, ttlSeconds = 7 * 24 * 60 * 60) {
      try {
        ...
        // Fail-closed: if we can't record the revocation, throw so the
        // caller (logout) is aware the token may not be fully invalidated.
        throw err;
      }
    },
    async has(jti) {
      try {
        ...
        logger.error(
          "[tokenBlacklist] Redis has() error — failing CLOSED (token treated as revoked):",
          err.message
        );
        // Fail-closed: deny access when Redis is unreachable.
        // A brief service interruption is safer than allowing revoked tokens
        // to pass through on a healthcare platform.
        return true;
      }
    },
  };
...
if (redisUrl) {
  logger.info("[tokenBlacklist] Redis backend active (fail-closed on error).");
} else {
...
module.exports = {
   ...
   *  Returns true (deny) on Redis error — fail-closed for safety. */
